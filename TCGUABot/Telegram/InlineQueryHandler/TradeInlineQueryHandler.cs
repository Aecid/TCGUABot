...
{
    public class TradeInlineQueryHandler
    {
            ...
            foreach (var set in CardData.Instance.Sets)
            {
                ...
                if (cards.Count > 0)
                {
                    foreach (var card in cards)
                    {
                        ...
                        {
                            results.Add(new InlineQueryResultArticle((++k).ToString(), card.name, new InputTextMessageContent("/wtb_tcgid_" + card.tcgplayerProductId))
                            {
                                HideUrl = true,
                                ThumbWidth = 99,
                                ThumbHeight = 138,
        ...
        public List<InlineQueryResultArticle> GetCardsFromTcg(InlineQuery query)
        {
            ...
            if (cards.Count > 0)
            {
                ...
                foreach (var card in cards)
                {
                    results.Add(new InlineQueryResultArticle((++k).ToString(), card.name, new InputTextMessageContent("/wtb_tcgid " + card.productId))
                    {
                        HideUrl = true,
                        ThumbWidth = 99,
                        ThumbHeight = 138,
